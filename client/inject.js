...
(() => {
  ...
  function triageLine(text) {
    return `tone ${heuristicScore(text)} of ${CONFIG.triageThreshold} needed,`
      + ` padding ${paddingScore(text)} of 1 needed, ${wordCount(text)} words`;
  }
  ...
  function diagnose(item) {
    ...
    const row = { sender, threadReply, state: item.getAttribute(ATTR_STATE) };
    ...
    const known = verdicts.get(hash(text));
    ...
    if (known === CLEAN) return { ...row, why: `read as written; ${triageLine(text)}` };
    if (known) return { ...row, why: 'the model read it and left it as written' };
    if (matchesAny(CONFIG.ignoreChannels, channelName())) return { ...row, why: 'channel is on the ignore list' };
    ...
    if (text.length > CONFIG.maxChars) {
      return { ...row, why: `too long: ${text.length} chars, over the ${CONFIG.maxChars} limit` };
    }
    if (!shouldAsk(text)) return { ...row, why: `left alone; ${triageLine(text)}` };
